...
from customer_complaint_agent.shared.tool import Tool, ToolArgument, ToolRegistry
...
def agent_decision_from_model_data(data: dict[str, object]) -> AgentDecision:
    ...
    model_decision = model_decision_from_data(data)

    return agent_decision_from_model_decision(model_decision)
...
def _create_tool_arguments_schema(
    ...
    return {
        ...
        "properties": {
            argument.name: {
                **_create_argument_property_schema(
                    argument.argument_type,
                    argument.description,
                ),
            }
